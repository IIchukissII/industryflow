...
- **Refined by:** ADR-0018 (spawner portability — decision 1's "per-user pod spawner" is realized as KubeSpawner on Kubernetes and DockerSpawner on Compose, so the per-user environment is a *pod or a container*; the isolation properties here are unchanged and binding under both); ADR-0015 (the DB principal of decision 6 — "short-lived, per-session, not shared" — is refined into a *standing* per-tenant `NOLOGIN` role assumed via the SQL proxy, with the short-lived/per-session property moved onto the capability handle; the end invariants (single-tenant, read-only, revocable, no shared secret in the kernel) are preserved); ADR-0020 (per-user work persistence — resolves decision 1's deferred "where authored notebooks live")
...
6. **Direct database access for the authoring profile is a short-lived, read-only, single-tenant principal — never a shared service role.** When an authoring environment needs to query data directly (rather than through the API), it is given a credential whose grants reach exactly the one tenant schema of the user who opened it, which is read-only, and which is short-lived rather than standing. It is provisioned per session, not shared, and it is distinct from every service role the platform already defines. The mechanism by which such a credential is delivered without placing a reusable secret inside user-controlled code is deferred (see Deferred decisions), but the properties — single-tenant, read-only, short-lived, non-shared — are decided here. *(Refined by ADR-0015: the **standing** and **non-shared** wording above was reversed in the realized design — direct access is a per-tenant `NOLOGIN` role that is **standing and shared** across a tenant's sessions, assumed via the SQL proxy, and the short-lived/per-session/revocable property moved onto the capability handle the kernel presents rather than the role itself. The end invariants decided here — single-tenant, read-only, revocable, no shared secret in the kernel — are preserved.)*
