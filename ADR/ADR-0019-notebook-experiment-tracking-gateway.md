...
- **ID:** ADR-0019
- **Status:** Accepted (rev 1 — decision 7's deferred tenant-facing browsing is **delivered as a read-path**: the session-authed, tenant-scoped experiment and run surface now stands beside the registered-models one, closing the last of "Tenant-scoped experiment & model UI" and the gap left when the `/api/mlflow/*` shim was removed. Rev 1 also records what that surface establishes: decision 1's "only" binds the kernel's credentials, not every speaker to the tracking server, and decision 5's namespace rule therefore has a second site of realisation)
- **Date:** 2026-06-28 (rev 1: 2026-07-16)
...
5. **The gateway forces every tracking, registry, and artifact operation into the caller's tenant namespace, and refuses any that would cross it (ADR-0013 dec 2-3 made concrete).** On the single shared MLflow store, the gateway namespaces experiments and registered models by tenant (a `tenant_<uuid>.` name prefix — a dot, because MLflow forbids `/` and `:` in registered-model names — that it adds on write and strips on read) and places artifacts under a per-tenant object-store prefix (`tenant_<uuid>/`, a storage path where `/` is allowed). It validates every request against the resolved tenant — rejecting any experiment name, run id, model name, or artifact path that resolves outside it — so the kernel sees plain names within its own tenant and cannot address another's. Runs, model versions, and their artifacts therefore carry one tenant dimension together (ADR-0013 dec 3).
...
- **Tenant namespace literal format.** *Resolved (implementation).* Experiment and registered-model **names** carry a `tenant_<uuid>.` prefix (a dot — MLflow forbids `/` and `:` in model names); artifact keys use `tenant_<uuid>/`. The tenant token is the full company UUID because the namespace is a security boundary: the UUID is unique and immutable, so it is collision-free and survives a company rename, where a human slug is neither. The gateway strips the prefix on every read, so the tenant-facing surface only ever shows plain names — **safe** (UUID boundary) and **user-friendly** (no prefix on the surface). A human-readable tenant label inside the operator-only MLflow UI is a future nicety; it needs the tenant's display name carried in the capability binding, which the gateway does not hold.
- **Tenant-scoped experiment & model UI.** *(rev 1 — rev 0 read "**Partly delivered** … tenant-scoped experiment/run browsing remains owed".)* **Delivered as a read-path.** The platform serves a session-authed, tenant-scoped **registered-models** read-path (`GET/PATCH /api/registered-models`, ml-service) that reuses these namespace rules to list a tenant's models with description + source-run metrics under stripped names, rendered by the frontend Models page; and, since rev 1, the matching **experiment and run** read-path, which lists a tenant's experiments and their runs with metrics and parameters under the same stripped names. Together these **replaced** an earlier `/api/mlflow/*` shim that set a tenant `search_path` on MLflow's *shared* database — a no-op that returned every tenant's experiments and models — which is now removed. The experiment half of that removal stood unreplaced in the interim, so no tenant-facing experiment browsing existed at all between then and rev 1.

  Rev 1 records two consequences rather than leaving them to be re-derived. **The word "only" in decision 1 binds the kernel and its credentials — not the question of which component may address the tracking server.** These read-paths are trusted-side platform code (ADR-0013 dec 1) addressing it directly, which decision 7 anticipates in as many words: "the platform's own scoped API/UI". The gateway exists to mediate the *untrusted* kernel; reading decision 1 as a prohibition on any other speaker would forbid the surface decision 7 promises. **The tenant namespace rule of decision 5 accordingly has a second site of realisation:** the read-paths re-derive it on the trusted side instead of obtaining it from the gateway. This follows from decision 7 and is not drift — but decision 5 describes the gateway's enforcement, and no reader should infer from it that the gateway is the rule's only home. Whether the two should converge is left open.

- **Tenant-facing browsing beyond a read-path.** What the *frontend* presents on top of the above — the analytics and experiment pages, and the hub embedding they imply — is not settled here; the ingress topology it touches belongs to ADR-0014's deferred verification-handoff and single-origin question.
