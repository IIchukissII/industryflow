...
- **Tenant-scoped experiment & model UI.** *Partly delivered.* The platform now serves a session-authed, tenant-scoped **registered-models** read-path (`GET/PATCH /api/registered-models`, ml-service) that reuses these namespace rules to list a tenant's models with description + source-run metrics under stripped names, rendered by the frontend Models page. Tenant-scoped **experiment/run** browsing remains owed (inherits ADR-0013). This read-path **replaced** an earlier `/api/mlflow/*` shim that set a tenant `search_path` on MLflow's *shared* database — a no-op that returned every tenant's experiments and models — which is now removed.
