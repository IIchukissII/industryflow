...
**Revision 1.** Practice since acceptance surfaced a gap in decision 5. Several Accepted ADRs (e.g. ADR-0009, ADR-0010, ADR-0019) grew short, clearly-labeled sections recording what was *resolved or implemented* after acceptance — outcomes and resolved deferrals, not new decisions — which decision 5 as written sanctioned only on a still-Proposed draft. Forcing full supersession ceremony onto every post-acceptance status note discourages the very act of keeping the record current, while leaving the practice unaddressed lets it blur into silent decision edits. This revision draws the line explicitly: a labeled status addendum that changes no recorded decision is allowed on an Accepted ADR; anything that adds or alters a *decision* still takes a revision.
