...
# Test parameters
TARGET_MSG_PER_SEC = 1500
TEST_DURATION_SEC = 300
RESULTS_FILE = 'performance_results_W2_1500_reverted.csv'
...
async def send_measurement(session, sensor_id, value, unit, timestamp, headers, monitor):
    """Send single measurement asynchronously"""
    payload = {
        'timestamp': timestamp,
        'sensor_id': sensor_id,
    ...
    start = time.time()
    try:
        async with session.post(INGESTION_URL, json=payload, headers=headers, timeout=aiohttp.ClientTimeout(total=10)) as response:
            latency_ms = (time.time() - start) * 1000
            
            if response.status == 202:
                await monitor.record_success(latency_ms)
                return True
            else:
...
async def worker(queue, session, headers, monitor):
    """Worker that processes messages from queue"""
    while True:
        try:
            sensor_id, value, unit, timestamp = await asyncio.wait_for(queue.get(), timeout=1.0)
            await send_measurement(session, sensor_id, value, unit, timestamp, headers, monitor)
            queue.task_done()
...
async def main():
    print("=" * 80)
    print("IndustryFlow Async Performance Test")
    print("=" * 80)
    ...
    print(f"\n🚀 Starting async test... (interval: {interval*1000:.2f}ms)")
    ...
    async with aiohttp.ClientSession() as session:
        # Start workers
        workers = [asyncio.create_task(worker(queue, session, headers, monitor)) 
        ...
        try:
            ...
            while (time.time() - test_start) < TEST_DURATION_SEC:
                ...
                if sensor_id:
                    ...
                    await queue.put((sensor_id, value, unit, timestamp))
                
                sensor_idx += 1
                
                # Log every 10 seconds
                current_time = time.time() - test_start
                if int(current_time) % 10 == 0 and current_time - last_log_time >= 10:
                    ...
                    print(f"[{stats['elapsed']:.0f}s] "
                          ...
                          f"Latency: {stats['latency_avg']:.1f}ms (p95: {stats['latency_p95']:.1f}ms) | "
