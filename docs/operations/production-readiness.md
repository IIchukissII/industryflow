...
|---|------|----------|-----|--------|
| 1 | Release | HIGH → **mostly done** | `images.yml` builds all 13 images, Trivy-**gates on fixable CRITICAL** (`exit-code 1`), pip-audit/npm-audit, pushes `:latest`+`:sha-<short>` to GHCR on main. CRITICALs remediated: `mlflow` 2.8/2.9→2.17.2, `python-jose` 3.3→3.4, `pyarrow` (transitive ≥14). Spark-distro JARs (avro/derby/zookeeper) accepted in `.trivyignore` (justified, expiry) pending a Spark upgrade. **Remaining:** (a) upgrade Spark to drop the `.trivyignore` entries; (b) pin Helm to immutable **digests** (still `:latest`); (c) per-image path filters | Spark upgrade; digest-pin Helm |
