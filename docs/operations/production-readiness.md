...
| 5 | Containers | HIGH → **done** | **All** first-party images run **non-root** with a restricted `securityContext` (`runAsNonRoot`, drop **all** caps, `allowPrivilegeEscalation=false`, seccomp `RuntimeDefault`, read-only root FS + writable scratch where applicable): app/infra/db-backup as uid 10001, Spark uid 185, and the **frontend** now an **unprivileged nginx** (uid 101, listens 8080/8443). **Remaining:** the spawned **jupyter** image still uses `--allow-root` — set KubeSpawner `runAsUser` when notebooks are cluster-deployed | Set KubeSpawner `runAsUser` for jupyter |
