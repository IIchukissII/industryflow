...
| 4 | Secrets | HIGH → **mostly done** | Done: Helm renders a **scoped Secret per workload** (only the keys it needs — no pod gets the full bundle), each service connects as its **own least-privilege DB role** (not the superuser), and `helm install` **fails closed** on un-overridden `CHANGE_ME` placeholders (`secrets.failOnPlaceholders`). **Remaining:** `pg_hba` still allows superuser/replication from `0.0.0.0/0` and there's no DB TLS; compose still publishes DB/Kafka/Redis host ports | Lock down `pg_hba` (scope to private nets, then `hostssl`+scram); bind compose host ports to localhost |
